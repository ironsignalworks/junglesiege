...
export function updateCanvasSize({ keepTankPosition = true } = {}) {
  ...
  if (keepTankPosition && tank) {
    ...
    const hudHeight = state.bottomBarHeight || 32;
    const hudMargin = 8; // match .hud-bottom { bottom: 8px; }
    const tankOffset = 12; // extra offset to move tank further down
    tank.y = Math.max(0, Math.min(tank.y, drawH - hudHeight - hudMargin - tank.height + tankOffset));
  }
}
...
export function resetGame() {
  ...
  // Reset tank position
  if (tank) {
    const hudHeight = state.bottomBarHeight || 32; // fallback to 32px if not set
    const hudMargin = 8; // match .hud-bottom { bottom: 8px; }
    const tankOffset = 12; // extra offset to move tank further down
    tank.x = canvas.width / 2 - tank.width / 2;
    tank.y = canvas.height - hudHeight - hudMargin - tank.height + tankOffset;
  }
